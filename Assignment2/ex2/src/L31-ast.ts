...
import { makeEmptySExp, makeSymbolSExp, SExpValue, makeCompoundSExp, valueToString, compoundSExpToString } from '../imp/L3-value'
...
//class
export const makeClassExp = (vars: VarDecl[], bindings: Binding[]) : ClassExp =>
    ({tag: "ClassExp", vars: vars, bindings: bindings})
...
export const parseL31SpecialForm = (op: Sexp, params: Sexp[]): Result<CExp> =>
    ...
    op === "class" ? parseClassExp(first(params), second(params)) : //first is [a,b] rest is [pair, sum]
    makeFailure("Never");
...
export const parseClassExp = (fields: Sexp, methods: Sexp): Result<ClassExp> =>{ 
    ...
    const methodDec = map(b => b[0], methods);
    const methodBodies = mapResult(method => parseL31CExp(second(method)), methods);
    const bindingsResult = bind(methodBodies, (vals: CExp[]) => makeOk(zipWith(makeBinding, methodDec, vals)));
    const fieldsResult: Result<VarDecl[]> = isArray(fields) && allT(isString, fields)? mapResult(field => makeOk(makeVarDecl(field)), fields) : makeFailure("bad fields")
    return safe2((fields: VarDecl[] ,bindings: Binding[]) => makeOk(makeClassExp(fields, bindings)))
...
const unparseClassExp = (cl: ClassExp) : string =>
    `(class (${map((p: VarDecl) => p.var, cl.vars).join(" ")}) (${map((b:Binding) => `(${b.var.var} ${unparseL31(b.val)})`, cl.bindings).join(" ")}))`
